...
class ALBEF(nn.Module):
    ...
    def inference(self, image, question=None, use_embeds=False):
        # image_embeds = self.visual_encoder(image)
        # image_atts = torch.ones(image_embeds.size()[:-1],dtype=torch.long).to(image.device)
        ...
        # return {'fusion_output': question_output.last_hidden_state}
        if not use_embeds:
            text_output = self.text_encoder(question.input_ids, attention_mask=question.attention_mask,
                                            mode='text')
        else:
            text_output = self.text_encoder(inputs_embeds=question.input_ids, attention_mask=question.attention_mask,
                                            mode='text')
